...
let logicJS = (brd) => {
    ...
    var graph = brd.create('functiongraph',
        [function (x) { return 2 * Math.cos(x); }, -10, 10], { strokeColor: 'green', strokeWidth: 2 }
    );
    // Left point
    ...
    // Point on cos curve
    var p4 = brd.create('point', [function () { return degrees_to_radians(theta.Value()); }, function () { return 2 * Math.cos(degrees_to_radians(theta.Value())); }], { name:  function () { return Number(2 * Math.cos(degrees_to_radians(theta.Value()))).toFixed(2);}, withLabel: true, visible: true, strokeColor: 'black', strokeWidth: 1, size: 3 });
    ...
    var is2 = brd.create('intersection', [per0, axis_X, 0], { name: function () { return Number(2 * Math.cos(degrees_to_radians(theta.Value()))).toFixed(2); }, withLabel: true, visible: true, strokeColor: 'blue', strokeWidth: 2, size: 0 });
    var s2 = brd.create('segment', [p0, is2], { strokeColor: 'red', strokeWidth: 2 });
